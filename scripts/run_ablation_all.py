#!/usr/bin/env python3
"""
Run ablation on all 17 circuits.
  B) no_ptt:    ATPG_PER_TARGET_TIMEOUT=0
  C) no_tp_T2:  --t2-two-phase off
Output: results/ablation_{no_ptt,no_tp_T2}.csv
"""
...
def fresh_start():
    """Archive existing summary so next run starts fresh."""
    ...
    # Clear residual artifacts from previous runs
...
if __name__ == "__main__":
    print("=" * 60)
    print("Ablation: all 17 circuits")
    print("=" * 60)

    print("\n>>> Condition B: no_ptt (ptt=0) <<<")
    run_one_condition("no_ptt", env_vars={"ATPG_PER_TARGET_TIMEOUT": "0"})

    print("\n>>> Condition C: no_tp_T2 (Two-Phase OFF at T=2) <<<")
